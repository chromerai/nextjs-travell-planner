...
export default function Trips() {
  ...
  const renderCell = React.useCallback((trip: TripType, columnKey: React.Key) => {
    const cellValue = trip[columnKey as keyof TripType];

    switch (columnKey) {
        case "id":
            return <Link href={`/trips/${cellValue}`} target="_blank">
                {cellValue as string}
            </Link>
        case "scrappedOn":
        ...
        case "destinationItinerary": {
            ...
            if(Array.isArray(cellValue)) {
                const itineraryValues = cellValue.slice(0, 4) as {place: string}[];
                return (
